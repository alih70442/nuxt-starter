<template>
  ...
  >
    ...
    >
      <section class="mt-4">
        <ul class="flex mt-3">
          <li class="basis-0 grow ml-2">نام</li>
        ...
        <ul class="space-y-3 mt-1">
          ...
          >
            <div class="basis-0 grow ml-2">
              {{ product_variation.variation_types.map(v => v.title).join(' - ') ?? 'بدون ویژگی' }}
            </div>
...
import { cloneDeep, groupBy, isNumber, xor } from 'lodash';
...
const is_variation_edited = ref(false);
const ref_form = ref();
const showMoreSettings = ref(false);

const { is_loading, data, fetch } = useFetch(show_supplier_product, {
  manual: true,
});
  ...
  onShow(passed_id) {
    ...
    fetch({
      ...
      setTimeout(async () => {
        ...
        await onCategoryChanged(rs_data.category_id);
      });
    });
    ...
    index_preparation_days().then(rs => {
      const rs_data = get_response_data(rs);
      inputs.value.preparation_days.options = [
        {
          id: '',
          title: 'ندارد',
        },
...
const extract_variation_type_values = variation_types => {
  const groupedByType = variation_types.reduce((result, variation) => {
    variation.variation_types.forEach(({ type, value }) => {
      if (!result[type.id]) {
        result[type.id] = new Set();
      }
      result[type.id].add(value.id);
    });
    return result;
  }, {});
  ...
  return Object.fromEntries(
    Object.entries(groupedByType).map(([id, values]) => [id, Array.from(values)])
  );
};
...
const onCategoryChanged = category_id => {
  return fetch_category({
    ...
    if (inputs.value.variations.value.length === rs_data.variation_types.length) return;

    is_variation_edited.value = true;

    inputs.value.variations.value = rs_data.variation_types.map(vt => ({
      id: vt.id,
      list: [
        {
          id: unique_id(),
...
const go_variations = () => {
  const variation_type_values = extract_variation_type_values(data.value.product_variations);

  if (Object.keys(variation_type_values).length) {
    inputs.value.variations.value = Object.keys(variation_type_values).map(vtv => ({
      id: vtv,
      list: variation_type_values[vtv].map(value => ({
        id: get_unique_id(),
        value,
      })),
    }));
...
const go_product_variations = () => {
  ...
  data.value.product_variations.forEach(pv => {
    const found = inputs.value.product_variations.value.find(innerPv => {
      return pv.variation_types.every(vt => {
        return innerPv.variation_types.some(
          innerType => innerType.id == vt.type.id && innerType.value == vt.value.id
        );
      });
    });
    ...
    if (found) {
      found.id = pv.id;
      found.stock = pv.stock;
      found.$price = pv.$price;
    }
  });
...
const submit = async () => {
  const product_variations = inputs.value.product_variations.value.map(pv => ({
    ...(isNumber(pv.id) && { id: pv.id }),
    stock: pv.stock,
    $price: pv.$price,
    variation_types: pv.variation_types,
  }));
